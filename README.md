...
```
pack build node-function-rhos --path . --buildpack paketo-buildpacks/nodejs --builder paketobuildpacks/builder:base
```

If using Jammy:

```
pack build node-function-rhos --path . --buildpack paketo-buildpacks/nodejs --builder paketobuildpacks/builder-jammy-buildpackless-base:latest
```

Where `node-function-rhos` is the name of your runnable function image.

Then run via Docker:

```
docker run -it --rm -p 8080:8080 node-function-rhos
```
...
```
tanzu apps workload create node-function-rhos -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/node-function-rhos-source \
  --type web
```
...
```
tanzu apps workload get node-function-rhos
```
...
```
export SOURCE_IMAGE=registry/project/node-function-rhos
