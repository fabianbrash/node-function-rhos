...
k8s_custom_deploy(
    ...
    apply_cmd="tanzu apps workload apply -f config/workload.yaml --update-strategy replace --debug --live-update" +
        ...
        " --namespace " + NAMESPACE +
        " --yes >/dev/null" +
        " && kubectl get workload node-function-rhos --namespace " + NAMESPACE + " -o yaml",
...
k8s_resource('node-function-rhos', port_forwards=["8080:8080"],
    extra_pod_selectors=[{'carto.run/workload-name': 'node-function-rhos', 'app.kubernetes.io/component': 'run'}])
